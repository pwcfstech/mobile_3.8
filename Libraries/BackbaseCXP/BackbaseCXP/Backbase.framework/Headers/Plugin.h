 ...
 * the
 * native part is done and is ready to process the data gathered on the native land, or the native operation fail
 * and
 * for what reason.
 */
 ...
 * to
 * determine what to do if the required parameters weren't passed at the moment a plugin function is called.
 */
 ...
 */
- (void)success:(id)jsonCompatibleObject callbackId:(NSString*)callbackId;
 ...
 */
- (void)success:(id)jsonCompatibleObject callbackId:(NSString*)callbackId keep:(BOOL)keep;
 ...
 * @param callbackId The callbackId that this invocation refers to.
 */
- (void)error:(id)jsonCompatibleObject callbackId:(NSString*)callbackId;
 ...
 * @param keep Keep the callbackId in the queue, by default it's removed
 */
- (void)error:(id)jsonCompatibleObject callbackId:(NSString*)callbackId keep:(BOOL)keep;

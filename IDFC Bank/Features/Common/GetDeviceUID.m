...
+ (NSString *)uid {
    return [[[GetDeviceUID alloc] initWithKey:@"deviceUID"] uid];
}
 ...
 */
- (void)save {
    if (![GetDeviceUID valueForUserDefaultsKey:_uidKey]) {
        [GetDeviceUID setValue:_uid forUserDefaultsKey:_uidKey];
    }
    if (![GetDeviceUID valueForKeychainKey:_uidKey service:_uidKey]) {
        [GetDeviceUID setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
    }
}
